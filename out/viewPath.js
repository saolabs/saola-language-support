...
exports.ALIAS_DIRS = void 0;
exports.splitViewPath = splitViewPath;
 ...
 * the alias directory.
 */
function pickBestCandidate(paths, dirs, aliasDir) {
    let best;
    let bestScore = -1;
    for (const path of paths) {
        ...
        for (let i = 1; i <= dirs.length && i <= candidateDirs.length; i++) {
            if (candidateDirs[candidateDirs.length - i] !== dirs[dirs.length - i]) {
                break;
            }
            run++;
        }
        ...
        if (aliasDir && candidateDirs.includes(aliasDir)) {
            score += 0.5;
        }
        if (score > bestScore) {
            bestScore = score;
...
//# sourceMappingURL=viewPath.js.map