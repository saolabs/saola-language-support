...
exports.CLOSURE_NAMES = void 0;
exports.maskComments = maskComments;
exports.setupBlock = setupBlock;
exports.buildVirtual = buildVirtual;
...
function objectTypeColon(inner) {
...
function names(inner) {
    ...
    for (let i = 0; i < body.length; i++) {
        ...
        if (c === ',') {
            add(i);
            start = i + 1;
            inType = inValue = false;
        }
    }
    add(body.length);
    return out;
}
const setterOf = (name) => 'set' + name.charAt(0).toUpperCase() + name.slice(1);
/** `{a: 1}` (old form) has a `:` directly inside the outer braces; nested `{a = {x: 1}}` does not. */
function usesColonDefaults(object) {
    let depth = 0, quote = '';
...
function buildVirtual(source) {
    const masked = maskComments(source);
    const block = setupBlock(masked) ?? { body: '', offset: 0 };
    const b = new Builder();
    for (const [name, type] of Object.entries(exports.CLOSURE_NAMES)) {
        b.gen(`declare const ${name}: ${type};\n`);
    }
    const anyNames = new Set();
    const states = [];
    const computed = [];
    const emit = (text, offset, declarationsOnly) => {
        let pos = 0;
        ...
        for (let m = DECLARATION.exec(text); m; m = DECLARATION.exec(text)) {
            ...
            if (!declarationsOnly) {
                b.copy(text.slice(pos, m.index), offset + pos);
            }
            ...
            if (kind === 'importView') {
                ...
                if (usesColonDefaults(defaults)) {
                    ...
                    if (kind.startsWith('state')) {
                        names(defaults).forEach(n => anyNames.add(setterOf(n)));
                    }
                }
                else {
                    b.gen('let ').copy(defaults, offset + innerStart);
                    if (colon >= 0) {
                        b.gen(': Partial<').copy(inner.slice(colon + 1), offset + innerStart + colon + 1).gen('>');
                    }
                    b.gen(' = __data__;');
                    if (kind.startsWith('state')) {
                        states.push(...names(defaults));
                    }
                }
            }
        ...
        if (!declarationsOnly) {
            b.copy(text.slice(pos), offset + pos);
        }
    };
    ...
    b.gen('\nexport {};\n');
    for (const n of anyNames) {
        b.gen(`declare let ${n}: any;\n`);
    }
    for (const n of states) {
        b.gen(`declare function ${setterOf(n)}(value: typeof ${n}): void;\n`);
    }
    for (const n of computed) {
        b.gen(`declare function get$${n}(): typeof ${n};\n`);
    }
    return b;
}
function checkSetup(source) {
    ...
    for (const d of ts.getPreEmitDiagnostics(program)) {
        if (d.category !== ts.DiagnosticCategory.Error || d.file?.fileName !== fileName || d.start === undefined || IGNORED.has(d.code)) {
            continue;
        }
...
function viewScope(source) {
    const scope = new Set();
    for (const g of TEMPLATE_GLOBALS) {
        scope.add(g);
        scope.add('$' + g);
    }
    ...
    for (const st of file.statements) {
        if (ts.isVariableStatement(st)) {
            st.declarationList.declarations.forEach(d => bind(d.name));
        }
            ...
            if (nb) {
                ts.isNamespaceImport(nb) ? bind(nb.name) : nb.elements.forEach(e => bind(e.name));
            }
        }
...
function arrowParams(tokens) {
    const K = ts.SyntaxKind;
    const params = new Set();
    tokens.forEach((t, j) => {
        if (t.kind !== K.EqualsGreaterThanToken || j === 0) {
            return;
        }
        const before = tokens[j - 1];
        if (before.kind === K.Identifier) {
            params.add(before.text);
            return;
        }
        if (before.kind !== K.CloseParenToken) {
            return;
        }
        let depth = 0;
        for (let i = j - 1; i >= 0; i--) {
            if (tokens[i].kind === K.CloseParenToken) {
                depth++;
            }
            else if (tokens[i].kind === K.OpenParenToken && --depth === 0) {
                break;
            }
            else if (tokens[i].kind === K.Identifier) {
...
function reads(tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (t.kind === K.OpenBraceToken) {
            braces++;
        }
        else if (t.kind === K.CloseBraceToken) {
            braces--;
        }
        if (t.kind !== K.Identifier || params.has(t.text)) {
            continue;
        }
        if (prev && (prev.kind === K.DotToken || prev.kind === K.QuestionDotToken)) {
            continue;
        }
        if (next && next.kind === K.OpenParenToken) {
            continue;
        }
        if (braces > 0 && next && next.kind === K.ColonToken && prev && (prev.kind === K.OpenBraceToken || prev.kind === K.CommaToken)) {
            continue;
        }
        out.push(t);
    }
...
function checkTemplate(source) {
    ...
    for (let m = directive.exec(region); m; m = directive.exec(region)) {
        ...
        if (TEMPLATE_DECLARATIONS.test(m[1])) {
            ...
            names(inner).forEach(n => { scope.add(n); if (/^states?$/.test(m[1])) {
                scope.add(setterOf(n));
            } });
